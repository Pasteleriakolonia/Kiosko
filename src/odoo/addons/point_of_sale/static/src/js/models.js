odoo.define("point_of_sale.models", function(require) {
  ...
  var load_invoice_data = function() {
    rpc
      .query({
        model: "pos.order",
        method: "invoice_data",
      ...
      .then(function(response) {
        if (typeof response != "undefined") {
          window.localStorage.setItem("invoice_data", JSON.stringify(response));
        }
      });
  };
      ...
      },
      {
        ...
        loaded: function(self, configs) {
          ...
          for (var i = 0; i < orders.length; i++) {
            self.pos_session.sequence_number = Math.max(
      ...
      },
      {
        ...
        domain: function(self) {
          return [
            ...
            "|",
            ["groups_id", "=", self.config.group_pos_manager_id[0]],
    ...
    render_html_for_customer_facing_display: function() {
      ...
      if (order) {
        order.get_orderlines().forEach(function(orderline) {
          ...
          var image_url =
            window.location.origin + "/web/image?model=product.product&field=image_medium&id=" + product.id;
      ...
      return $.when.apply($, get_image_deferreds).then(function() {
        ...
        $rendered_html
          .find(".pos-total")
          .find(".pos_total-amount")
          .html(order_total_with_tax);
        ...
        rendered_html =
          QWeb.render("CustomerFacingDisplayHead", {
    ...
    _save_to_server: function(orders, options) {
      ...
      return rpc
        ...
        .then(function(server_ids) {
          console.log("After save orders load information");
          load_invoice_data();
  ...
  exports.load_models = function(models, options) {
    ...
    if (options.before) {
      for (var i = 0; i < pmodels.length; i++) {
        if (pmodels[i].model === options.before || pmodels[i].label === options.before) {
          index = i;
          break;
        }
      }
      ...
      for (var i = 0; i < pmodels.length; i++) {
        if (pmodels[i].model === options.after || pmodels[i].label === options.after) {
          index = i + 1;
        }
      }
    }
  ...
  exports.Product = Backbone.Model.extend({
    ...
    get_price: function(pricelist, quantity) {
      ...
      if (pricelist === undefined) {
        alert(
          _t("An error occurred when loading product prices. " + "Make sure all pricelists are available in the POS.")
        );
      }
  ...
  exports.Orderline = Backbone.Model.extend({
    ...
    can_be_merged_with: function(orderline) {
      var price = parseFloat(
        round_di(this.price || 0, this.pos.dp["Product Price"]).toFixed(this.pos.dp["Product Price"])
      );
      if (this.get_product().id !== orderline.get_product().id) {
        ...
        !utils.float_is_zero(
          price - orderline.get_product().get_price(orderline.order.pricelist, this.get_quantity()),
    ...
    _compute_all: function(tax, base_amount, quantity) {
      ...
      if (
        (tax.amount_type === "percent" && !tax.price_include) ||
  ...
  exports.Order = Backbone.Model.extend({
    ...
    export_as_JSON: function() {
      ...
      orderLines = [];
      this.orderlines.each(
        _.bind(function(item) {
          return orderLines.push([0, 0, item.export_as_JSON()]);
        }, this)
      );
      paymentLines = [];
      this.paymentlines.each(
        _.bind(function(item) {
    ...
    fix_tax_included_price: function(line) {
      if (this.fiscal_position) {
        ...
        if (mapped_included_taxes.length > 0) {
          unit_price = line.compute_all(mapped_included_taxes, unit_price, 1, this.pos.currency.rounding, true)
            .total_excluded;
        }
    ...
    // Get invoice data and save in localStorage
    get_invoice_data: function() {
      if (window.localStorage.hasOwnProperty("invoice_data")) {
        return JSON.parse(window.localStorage.getItem("invoice_data"));
      }
      return false;
    },
    /* ---- Screen Status --- */
