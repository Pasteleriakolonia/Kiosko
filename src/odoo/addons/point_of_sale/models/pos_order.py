...
# ESTAS 3 LIBRERIAS AGREGAS POR MANFRED... SON LIBRERIAS DE MANEJO DE XML EXISTENTES EN PYTHON
# ------------------------- *** --------------------------------------------------------------
...
class PosOrder(models.Model):
    ...
    def _get_account_move_line_group_data_type_key(self, data_type, values, options={}):
        ...
        if data_type == 'product':
            return ('product',
                    values['partner_id'],
                    (values['product_id'], tuple(
                        values['tax_ids'][0][2]), values['name']),
    ...
    def _reconcile_payments(self):
        for order in self:
            ...
            try:
                ...
                _logger.exception(
                    'Reconciliation did not work for order %s', order.name)
    ...
    statement_ids = fields.One2many('account.bank.statement.line', 'pos_statement_id', string='Payments', states={
    ...
    pos_reference = fields.Char(
        string='Receipt Ref', readonly=True, copy=False)
    ...
    def _onchange_amount_all(self):
        for order in self:
            ...
            order.amount_return = sum(
                payment.amount < 0 and payment.amount or 0 for payment in order.statement_ids)
    ...
    #   -----------  CODIGO AGREGADO POR MANFRED  --------------------------

    def dataOrder(self, value):
        response = self.env['pos.order'].search_read([['id', '=', value]])
        return response

    def dataCostumer(self, value):
        response = self.env['res.partner'].search_read([['id', '=', value]])
        return response

    def dataInvoice(self, value):
        response = self.env['account.invoice'].search_read(
            [['id', '=', value]])
        return response

    def dataProduct(self, value):
        response = self.env['pos.order.line'].search_read(
            [['order_id', '=', value]])
        return response

    #  Validación de NIT.
    def validateCostumer(self, value):
        url = "https://testgface.efactura.com.gt/mx.com.fact.wsfront/FactWSFront.asmx?wsdl"
        ...
        body = """<?xml version="1.0" encoding="UTF-8"?>
               ...
               <soapenv:Body>
                  <ws:RequestTransaction>
                     <!--Optional:-->
                     <ws:Requestor>99A6A4E6-DC02-484B-A12B-FB3D5F05C712</ws:Requestor>
                     <!--Optional:-->
                     <ws:Transaction>SEARCH_BASIC</ws:Transaction>
                     <!--Optional:-->
                     <ws:Country>GT</ws:Country>
                     <!--Optional:-->
                     <ws:Entity>83748687</ws:Entity>
                     <!--Optional:-->
                     <ws:User>99A6A4E6-DC02-484B-A12B-FB3D5F05C712</ws:User>
                     <!--Optional:-->
                     <ws:UserName>83748687</ws:UserName>
                     <!--Optional:-->
                     <ws:Data1></ws:Data1>
                     <!--Optional:-->
                     <ws:Data2>""" + str(value) + """</ws:Data2>
                     <!--Optional:-->
                     <ws:Data3></ws:Data3>
                  </ws:RequestTransaction>
               </soapenv:Body>
        ...
        ResponseData1Initial = response.text.find(str("<ResponseData1>"))
        ResponseData1End = response.text.find('</ResponseData1>')
        responseResponseData1 = response.text[ResponseData1Initial:ResponseData1End]
        dato1ResponseData1 = responseResponseData1.split("<ResponseData1>")
        NewResponse = []
        NewResponse = dato1ResponseData1[1].split('|')
        UserError(_(response))
        return NewResponse

    def calculateTax(self, price, subtotal, quantity):
        total = price * quantity
        taxTotal = total / 1.12
        NewTotal = total - taxTotal
        return price, quantity, total, taxTotal, NewTotal

    def structDetailXml(self, ProductNombre, newprice):
        response = """<Detalle>
                        <CodigoIdentificacion>000</CodigoIdentificacion>
                        <Categoria>SERVICIO</Categoria>
                        <Cantidad>""" + str(newprice[1]) + """</Cantidad>
                        <UnidadMedida>UNI</UnidadMedida>
                        <Descripcion>""" + ProductNombre + """</Descripcion>
                        <PrecioUnitario>""" + str(newprice[0]) + """</PrecioUnitario>
                        <Monto>""" + str(newprice[2]) + """</Monto>
                        <Descuento>0</Descuento>
                        <MontoDespuesDeDescuento>""" + str(newprice[2]) + """</MontoDespuesDeDescuento>
                        <Impuestos>
                            <Impuesto>
                                <NombreCorto>IVA</NombreCorto>
                                <CodigoUnidadGravable>1</CodigoUnidadGravable>  
                                <MontoGravable>""" + str("{0:.2f}".format(newprice[3])) + """</MontoGravable>
                                <MontoImpuesto>""" + str("{0:.2f}".format(newprice[4])) + """</MontoImpuesto>
                            </Impuesto>
                        </Impuestos>            
                    </Detalle>"""
    ...
    def structTotalXml(self, total, pricetotal, basetotal, totaltax, txtmoney, idOrder):
        response = """<Totales>
                    <TotalDeDescuento>0</TotalDeDescuento>
                    <SubTotalMenosDescuento>""" + str(total) + """</SubTotalMenosDescuento>
                    <ImpuestosTotales>
                        <ImpuestoTotal NombreCorto="IVA" TotalMontoImpuesto='""" + str("{0:.2f}".format(totaltax)) + """'/>
                    </ImpuestosTotales>
                ...
                <Adenda>
                    <CamposAdionales>
                        <EncabezadoAdicional>
                            <NotasDePie>
                                <TotalEnLetras>""" + txtmoney + """</TotalEnLetras>
                                <IdInterno>""" + str(idOrder) + """</IdInterno>
                                <Tipodecambio>7.79</Tipodecambio>
                                <Nota>.</Nota>                
                            </NotasDePie>
                        </EncabezadoAdicional>
                        <DetallesAdicional>
                            <DetalleAdicional>                    
                                <InformacionAdicional>INFORMACION ADICIONAL A CADA ITEM</InformacionAdicional>
                            </DetalleAdicional>
                        </DetallesAdicional>
                    </CamposAdionales>
                </Adenda>"""
    ...
    def numero_to_letras(self, numero):
        ...
        while entero > 0:
            a = entero % 1000
            if contador == 0:
                en_letras = order.convierte_cifra(a, 1).strip()
            else:
                en_letras = order.convierte_cifra(a, 0).strip()
            if a == 0:
        ...
        newNumero_Letra = numero_letras.replace("con 0/100.", "Exactos.")
        if decimal == 0:
            RealNumero = newNumero_Letra
        else:
            RealNumero = numero_letras
        return RealNumero
    ...
    def convierte_cifra(self, numero, sw):
        ...
        lista_decena = [
            ...
            ("DIEZ", "ONCE", "DOCE", "TRECE", "CATORCE", "QUINCE", "DIECISEIS",
             "DIECISIETE", "DIECIOCHO", "DIECINUEVE"), ("VEINTE", "VEINTI"),
    ...
    def Base64InvoiceXML(self, data):
        response = data.encode("UTF-8")
        response1 = base64.b64encode(response)
        base64String = str(response1)
        NewString = base64String.split("b'")
        NewStringVal = NewString[1].split("'")
        #raise UserError(_(str(NewStringVal[0])))
        return NewStringVal[0]
    #  ----------------------  fin codigo MANFRED  ---------------------------
    ...
    def action_pos_order_invoice(self):
        ...
        for order in self:
            ...
            dataList = order._prepare_invoice()
            responseOrder = order.dataOrder(order.id)
            responseCostumer = order.dataCostumer(
                responseOrder[0]['partner_id'][0])
            #dataValidation = order.validateCostumer(responseCostumer[0]['vat'])
            responseInvoice = order.dataInvoice(
                responseOrder[0]['invoice_id'][0])
            responseProduct = order.dataProduct(order.id)
            xmlDetailProduct = ""
            total = responseInvoice[0]['amount_total']
            priceTotal = 0.0
            TaxTotal = 0.0
            baseTotal = 0.0
            textMoney = ""
            for x in range(0, len(responseProduct)):
                NewPrices = order.calculateTax(
                    responseProduct[x]['price_unit'], responseProduct[x]['price_subtotal'], responseProduct[x]['qty'])
                xmlDetailProduct += order.structDetailXml(
                    responseProduct[x]['product_id'][1], NewPrices)
                priceTotal = priceTotal + NewPrices[0]
                baseTotal = baseTotal + NewPrices[3]
                TaxTotal = TaxTotal + NewPrices[4]

            textMoney = order.numero_to_letras(total)
            xmlTotal = order.structTotalXml(
                total, priceTotal, baseTotal, TaxTotal, textMoney, order.id)
            dateInvoice = datetime.datetime.now()
            date = str(dateInvoice.isoformat())
            newDate = []
            newDate = date.split(".")
            # print(newDate[0])
            #raise UserError(_(responseProduct))
# --------------------------------------------------------------------------------------------------------------------------------
            ...
            <ediFactura xmlns="Schema-ediFactura"
                xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                xsi:schemaLocation="Schema-ediFactura https://fel.g4sdocumenta.com/esquemas/NativoSchemav2.xsd">
                <Version>2</Version>
                <Encabezado>
                    <TipoDocumento>FACT</TipoDocumento>
                    <FechaEmision>""" + str(newDate[0]) + """</FechaEmision>
                    <CodigoMoneda>GTQ</CodigoMoneda>            
                    <IDInterno>""" + str(order.id) + """</IDInterno>
                </Encabezado>
                <Emisor>
                    <NIT>88466442</NIT>
                ...
                <Comprador>
                    <NITComprador>""" + str(responseCostumer[0]['vat']) + """</NITComprador>
                    <NombreComprador>""" + str(responseCostumer[0]['name']) + """</NombreComprador>
                    <DireccionComprador>
                        <Direccion>""" + str(responseCostumer[0]["street"]) + """</Direccion>
                        <CodigoPostal>01001</CodigoPostal>
                        <Municipio>""" + "Guatemala" + """</Municipio>
                        <Departamento>""" + str(responseCostumer[0]["city"]) + """</Departamento>
                        <Pais>GT</Pais>
                    </DireccionComprador>
                    <Telefono>""" + str(responseCostumer[0]["phone"]) + """</Telefono>
                </Comprador>
                <CorreoElectronico>
                    <De>usuario@gmail.com</De>
                    <Para>""" + str(responseCostumer[0]['email']) + """</Para>
                    <Copia>lopezStuard3@gmail.com</Copia>
                    <Oculto>oculto@gmsil.com</Oculto>
                    <Asunto>Facturacion Electronica</Asunto>
                    <Adjuntos>XML PDF</Adjuntos>
                </CorreoElectronico>
                <Detalles>
                """ + str(xmlDetailProduct) + """
                </Detalles>
                """ + str(xmlTotal) + """  
            </ediFactura>"""
            ...
            ConvertData = order.Base64InvoiceXML(data)
            ...
            body = """<?xml version="1.0" encoding="UTF-8"?>
                    <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ws="http://www.fact.com.mx/schema/ws">
                       ...
                       <soapenv:Body>
                          <ws:RequestTransaction>
                             <!--Optional:-->
                             <ws:Requestor>4076EC9D-6941-4726-8D07-3FB6C64BA3F4</ws:Requestor>
                             <!--Optional:-->
                             <ws:Transaction>SYSTEM_REQUEST</ws:Transaction>
                             <!--Optional:-->
                             <ws:Country>GT</ws:Country>
                             <!--Optional:-->
                             <ws:Entity>88466442</ws:Entity>
                             <!--Optional:-->
                             <ws:User>8E742EC3-CD9C-424A-9222-D4ECB1D6897A</ws:User>
                             <!--Optional:-->
                             <ws:UserName>Desarrollo</ws:UserName>
                             <!--Optional:-->
                             <ws:Data1>POST_DOCUMENTGT</ws:Data1>
                             <!--Optional:-->
                             <ws:Data2><![CDATA[""" + ConvertData + """]]></ws:Data2>
                             <!--Optional:-->
                             <ws:Data3>PDF</ws:Data3>
                          </ws:RequestTransaction>
                       </soapenv:Body>
                    </soapenv:Envelope>"""
            ...
            SuggestedFileNameInitial = response.text.find(
                str("<SuggestedFileName>"))
            SuggestedFileNameEnd = response.text.find('</SuggestedFileName>')
            responseSuggestedFileName = response.text[SuggestedFileNameInitial:SuggestedFileNameEnd]
            dato1SuggestedFileName = responseSuggestedFileName.split(
                "<SuggestedFileName>")

            BatchInitial = response.text.find(str("<Batch>"))
            BatchEnd = response.text.find('</Batch>')
            responseBatch = response.text[BatchInitial:BatchEnd]
            dato1Batch = responseBatch.split("<Batch>")

            SerialInitial = response.text.find(str("<Serial>"))
            SerialEnd = response.text.find('</Serial>')
            responseSerial = response.text[SerialInitial:SerialEnd]
            dato1Serial = responseSerial.split("<Serial>")
            
            # Code add by Josue Mente to parse and get data from WS
            base64DataStart = response.text.find("<ResponseData1>")
            base64DataEnd = response.text.find("</ResponseData1>")
            base64Data = response.text[base64DataStart:base64DataEnd]
            
            base64DataArray = base64Data.split("<ResponseData1>")
            
            if len(base64DataArray) < 2:
                raise UserError(_(response.text))
            
            
            base64String = base64DataArray[1]
            decodeXML = base64.b64decode(base64String)
            
            # Codigo añadido para recuperar las variables necesarias a imprimir en el voucher
            ...
            emisorAttrib = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:DatosEmision', namespaces).find('./dte:Emisor', namespaces).attrib

            nombreEmisor = emisorAttrib['NombreEmisor']

            direccionEmisor = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:DatosEmision', namespaces).find('./dte:Emisor', namespaces).find('./dte:DireccionEmisor', namespaces).find('./dte:Direccion', namespaces).text

            codigoPostalEmisor = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:DatosEmision', namespaces).find('./dte:Emisor', namespaces).find('./dte:DireccionEmisor', namespaces).find('./dte:CodigoPostal', namespaces).text

            municipioEmisor = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:DatosEmision', namespaces).find('./dte:Emisor', namespaces).find('./dte:DireccionEmisor', namespaces).find('./dte:Municipio', namespaces).text

            departamentoEmisor = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:DatosEmision', namespaces).find('./dte:Emisor', namespaces).find('./dte:DireccionEmisor', namespaces).find('./dte:Departamento', namespaces).text

            paisEmisor = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:DatosEmision', namespaces).find('./dte:Emisor', namespaces).find('./dte:DireccionEmisor', namespaces).find('./dte:Pais', namespaces).text

            nitEmisor = emisorAttrib['NITEmisor']

            tipoData = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:Certificacion', namespaces).find('./dte:NumeroAutorizacion', namespaces)

            serie = tipoData.attrib['Serie']
            numero = tipoData.attrib['Numero']
            numeroAutorizacion = tipoData.text

            fechaHoraCertificacion = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:Certificacion', namespaces).find('./dte:FechaHoraCertificacion', namespaces).text

            nombreCertificador = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:Certificacion', namespaces).find('./dte:NombreCertificador', namespaces).text

            nitCertificador = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:Certificacion', namespaces).find('./dte:NITCertificador', namespaces).text

            receptorData = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:DatosEmision', namespaces).find('./dte:Receptor', namespaces)

            idReceptor = receptorData.attrib['IDReceptor']
            nombreReceptor = receptorData.attrib['NombreReceptor']
            direccionReceptor = receptorData.find('./dte:DireccionReceptor', namespaces).find('./dte:Direccion', namespaces).text

            fechaDeEmision = root.find('./dte:SAT', namespaces).find('./dte:DTE', namespaces).find('./dte:DatosEmision', namespaces).find('./dte:DatosGenerales', namespaces).attrib['FechaHoraEmision']

            signatureValue = root.find('./ds:Signature', namespaces).find('./ds:SignatureValue', namespaces).text
            
            request.session['nombre_emisor'] = nombreEmisor
            request.session['nit_emisor'] = nitEmisor
            request.session['direccion_emisor'] = "%s %s %s %s %s" % (direccionEmisor, codigoPostalEmisor, municipioEmisor, departamentoEmisor, paisEmisor)
            request.session['serie'] = serie
            request.session['numero'] = numero
            request.session['numero_autorizacion'] = numeroAutorizacion
            request.session['fecha_certificado'] = dt.strptime(fechaHoraCertificacion, "%Y-%m-%dT%H:%M:%S").strftime("%d/%m/%Y %H:%M")
            request.session['nombre_certificador'] = nombreCertificador
            request.session['nit_certificador'] = nitCertificador
            request.session['id_receptor'] = idReceptor
            request.session['nombre_receptor'] = nombreReceptor
            request.session['direccion_receptor'] = direccionReceptor
            request.session['fecha_emision'] = dt.strptime(fechaDeEmision, "%Y-%m-%dT%H:%M:%S").strftime("%d/%m/%Y %H:%M")
            request.session['firma'] = signatureValue
...
class PosOrderLine(models.Model):
    ...
    order_id = fields.Many2one(
        'pos.order', string='Order Ref', ondelete='cascade')
    ...
    def _compute_amount_line_all(self):
        ...
        tax_ids_after_fiscal_position = fpos.map_tax(
            self.tax_ids, self.product_id, self.order_id.partner_id) if fpos else self.tax_ids
...
class ReportSaleDetails(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        data.update(self.get_sale_details(
            data['date_start'], data['date_stop'], configs))
        return data
